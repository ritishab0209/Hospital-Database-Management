...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!in_array($password['type'], $allowedTypes)) {
        ...
        echo "<script>window.location.href='doctor_login.php?message=".urlencode($message)."';</script>";
        exit;
    }
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>window.location.href='doctor_dashboard.php?message=".urlencode($message)."';</script>";
    } else {
